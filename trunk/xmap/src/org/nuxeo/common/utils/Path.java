...
public class Path implements Serializable {
     ...
     * <p>
     * The string path must represent a valid file system path on the local file
     * system.
     * <p>
     * The path is canonicalized and double slashes are removed except at the
     * beginning. (to handle UNC paths). All forward slashes ('/') are treated
     * as segment delimiters, and any segment and device delimiters for the
     * local file system are also respected (such as colon (':') and backslash
     ...
     * @param _separators
     */
    private Path(String[] segments, int _separators) {
        // no segment validations are done for performance reasons
        this.segments = segments;
        // hash code is cached in all but the bottom three bits of the
        // separators field
        this.separators = (computeHashCode() << USED_BITS)
                | (_separators & ALL_SEPARATORS);
    }

    /**
     * Creates a path object from an absolute and canonic path.
     * <p>
     * This method does not check the given path - it assumes the path has a
     * valid format of the form "/a/b/c" without duplicate slashes or dots.
     * 
     * @return the path
     */
    ...
    public Path append(Path tail) {
        ...
        Path result = new Path(newSegments, (separators & HAS_LEADING)
    ...
    private static String collapseSlashes(String path) {
        ...
        // if the path is only 0 or 1 chars long then it could not possibly have
        // illegal
        // duplicate slashes.
        if (length < 2) {
            return path;
