 ...
 * hierarchical in memory single-namespace naming system. A name in the
 * <code>MockContext</code> namespace is a sequence of one or more atomic names,
 * relative to a root initial context. When a name consist of more than one
 * atomic names it is a <code>CompoundName</code> where atomic names are
 * separated with separator character - '/' or '.'. It is possible to use both
 * separator characters in the same name. In such cases any occurrences of '.'
 * are replaced with '/' before parsing.
 * <p>
 * Leading and terminal components of a <code>CompoundName</code> can not be
 * empty - for example "name1/name2/name3" is a valid name, while the following
 * names are not valid - "/name1/name2/name3", "name1/name2/name3/",
 * "/name1/name2/name3/". If such name is passed, all empty leading/terminal
 * components will be removed before the name is actually used (this will not
 * affect the original value) - from the above three examples the actual name
 * will be "name1/name2/name3". If a name contains intermediate empty components
 * (for example "a//b") then <code>InvalidNameException</code> will be thrown.
 * <p>
 * Composite names (instances of <code>CompositeName</code>) must contain zero
 * or one component from the <code>MockContext</code> namespace.
 * <p>
 * The namespace of <code>MockContext</code> can be represented as a tree of
 * atomic names. Each name is bound to an instance of MockContext (subcontext)
 * or to an arbitrary object. Each subcontext has collection of names bound to
 * other subcontexts or arbitrary objects.
 * <p>
 * When instance of <code>Name</code> is used as parameter to any of the
 * MockContext methods, if the object is not <code>CompositeName</code> then it
 * is assumed that it is <code>CompoundName</code>
 * <p>
 * Example:
 * 
 * <pre>
 ...
 * <code>MockContextFactory</code>, when <code>InitialContext</code> is
 * instantiated.
 * <p>
 * If a remote context is provided, this class will search in that remote
 * context if the object is not found locally.
 * <p>
 * For overloaded methods that accept name as <code>String</code> or
 * <code>Name</code> only the version for <code>Name</code> is documented. The
 * <code>String</code> version creates <code>CompoundName</code>, from the
 * string name passed as parameter, and calls the <code>Name</code> version of
 * the same method.
 * 
...
public class MockContext implements Context {
    ...
    // MockContext supports single naming scheme and all instances use the same
    // parser.
     ...
     * context.
     */
    private final Map<Object, Object> objects = Collections
     ...
     * 
     * @param remoteContext
     *            remote context that MockContext will delegate to if it fails
     *            to lookup an object locally
     */
     ...
     *            to lookup an object locally
     * @param parent
     *            parent context of this context. <code>null</code> if this is
     *            the root context.
     * @param name
     *            atomic name for this context
     */
     ...
     * @param obj
     *            object to bind. Can be <code>null</code>.
     * 
     * @throws NoPermissionException
     *             if this context has been destroyed
     * @throws InvalidNameException
     *             if <code>name</code> is empty or is
     *             <code>CompositeName</code> that spans more than one naming
     *             system
     * @throws NotContextException
     *             if <code>name</code> has more than one atomic name and
     *             intermediate atomic name is bound to object that is not
     *             context.
     * 
    ...
    public void bind(Name name, Object obj) throws NamingException {
        checkIsDestroyed();
        // Do not check for already bound name. Simply replace the existing
        // value.
        rebind(name, obj);
    }
     ...
     *             if <code>name</code> is already bound in this Context
     * @throws NotContextException
     *             if any intermediate name from <code>name</code> is not bound
     *             to instance of <code>javax.naming.Context</code>
    ...
    public Context createSubcontext(Name name) throws NamingException {
        ...
        if (parsedName.size() == 1) {
            ...
            if (boundObject == null) {
                Context subContext = new MockContext(remoteContext, this,
                        subContextName);
                objects.put(subContextName, subContext);
                return subContext;
            } else {
                throw new NameAlreadyBoundException("Name " + subContextName
                        + " is already bound!");
            }
        } else {
            if (boundObject instanceof Context) {
                // Let the subcontext create new subcontext
                return ((Context) boundObject).createSubcontext(parsedName
                        .getSuffix(1));
            } else {
    ...
    /**
     * Destroys subcontext with name <code>name</code>. The subcontext must be
     * empty, otherwise <code>ContextNotEmptyException</code> is thrown.
     * <p>
     * Once a context is destroyed, the instance should not be used.
     * 
     * @param name
     *            subcontext to destroy
     * @throws NoPermissionException
     *             if this context has been destroyed
     * @throws InvalidNameException
     *             if <code>name</code> is empty or is
     ...
     *             if Context <code>name</code> is not empty
     * @throws NameNotFoundException
     *             if subcontext with name <code>name</code> can not be found
     * @throws NotContextException
     *             if <code>name</code> is not bound to instance of
    ...
    public void destroySubcontext(Name name) throws NamingException {
        ...
        if (boundObject == null) {
            throw new NameNotFoundException("Name " + subContextName
     ...
     *             if this context has been destroyed
     * @throws NamingException
     *             if any other naming error occurs
     ...
     *             if this context has been destroyed
     * @throws InvalidNameException
     *             if <code>name</code> is <code>CompositeName</code> that spans
     *             more than one naming system
     * @throws NameNotFoundException
     *             if <code>name</code> can not be found
     * @throws NotContextException
     *             component of <code>name</code> is not bound to instance of
     *             <code>MockContext</code>, when <code>name</code> is not an
     *             atomic name
     * @throws NamingException
     *             if any other naming error occurs
    ...
    public NamingEnumeration<Binding> listBindings(Name name)
        ...
        if (parsedName.isEmpty()) {
            ...
            for (Object o : objects.keySet()) {
                ...
                bindings.addElement(new Binding(bindingName, objects
                        .get(bindingName)));
            }
            ...
            if (subContext instanceof Context) {
                return ((Context) subContext).listBindings(parsedName
                        .getSuffix(1));
            }
     ...
     * @throws NotContextException
     *             component of <code>name</code> is not bound to instance of
     *             <code>MockContext</code>, when <code>name</code> is not
     *             atomic name.
     * @throws NamingException
     *             if any other naming error occurs
    ...
    /**
     * Rebinds object <code>obj</code> to name <code>name</code>. If there is
     * existing binding it will be overwritten.
     * 
     * @param name
     *            name of the object to rebind
     * @param obj
     *            object to add. Can be <code>null</code>
     * @throws NoPermissionException
     *             if this context has been destroyed
     * @throws InvalidNameException
     *             if <code>name</code> is empty or is
     ...
     *             system
     * @throws NameNotFoundException
     *             if intermediate context can not be found
     * @throws NotContextException
     *             if <code>name</code> has more than one atomic name and
     *             intermediate context is not found
     * @throws NamingException
     *             if any other naming exception occurs
    ...
    /**
     * Marks this context as destroyed. Method called only by
     * <code>destroySubcontext</code>.
     */
     ...
     * 
     * @param name
     *            <code>Name</code> to parse
     * @return parsed name as instance of <code>CompoundName</code>
     * @throws InvalidNameException
     *             if <code>name</code> is <code>CompositeName</code> and spans
     *             more than one name space
     * @throws NamingException
     *             if any other naming exception occurs
     */
    ...
    private class NamingEnumerationImpl implements NamingEnumeration<Binding> {
