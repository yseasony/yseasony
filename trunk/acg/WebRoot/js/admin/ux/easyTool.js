			...
			.extend(
					Ext.grid.EditorGridPanel,
					{
						...
						validateEx : function(ownerGrid) {
							...
							for ( var i = 0; i < m.length; i++) {
								...
								for ( var j = 0; j < fields.length; j++) {
									...
									var colIndex = ownerGrid.colModel
											.findColumnIndex(name);
									var rowIndex = ownerGrid.store
											.indexOfId(record.id);
									var editor = ownerGrid.colModel
											.getCellEditor(colIndex);
									if (editor != null) {
										editor = ownerGrid.colModel
												.getCellEditor(colIndex).field;
										if (!editor.validateValue(value)) {
											...
											cell = Ext.get(cell);
											cell.focus();
											ownerGrid.startEditing(rowIndex,
													colIndex);
											return false;
										}
									}
						...
						constructor : function(config) {
							...
							Ext.apply(this, config);
							Ext.ux.EditorGridPanelEx.superclass.constructor
									.call(this, arguments);
						}
					});
}
...

{
	...
	Ext.ux.RadioGroupEx = Ext.extend(Ext.form.Field, {
		...
		// private
		onResize : function() {
			Ext.ux.RadioGroupEx.superclass.onResize.apply(this, arguments);
			if (!this.boxLabel) {
		...
		onRender : function(ct, position) {
			Ext.ux.RadioGroupEx.superclass.onRender.call(this, ct, position);
		...
		setValue : function(v) {
			...
			if (!p)
				p = Ext.getBody();
			var target = p.child('input[name=' + escape(this.el.dom.name)
					+ '][value=' + v + ']', true);
			if (target)
				target.checked = true;
		},
		...
		disable : function() {
			...
			els.each(function(el) {
				if (el.dom.id == this.id) {
					Ext.ux.RadioGroupEx.superclass.disable.call(this);
				} else {
					var e = Ext.getCmp(el.dom.id);
					Ext.ux.RadioGroupEx.superclass.disable.call(e);
				}
			}, this);
		},
		...
		enable : function() {
			...
			els.each(function(el) {
				if (el.dom.id == this.id) {
					Ext.ux.RadioGroupEx.superclass.enable.call(this);
				} else {
					var e = Ext.getCmp(el.dom.id);
					Ext.ux.RadioGroupEx.superclass.enable.call(e);
				}
			}, this);
		},
...

{
	Ext.ux.Clock = function() {
		...
		this.display = function(ele) {
			...
			window.setTimeout(function() {clock.display(ele);
			}, 1000);
		};
	};
}
